...
from .metrics import AGG_DURATION, AGG_LAST_SUCCESS, AGG_RECORDS, AGG_RUNS
...
class AggregationCycle:
    ...
    def run_for_date(self, metric_date: date) -> dict:
        ...
        try:
            ...
            AGG_DURATION.observe(elapsed)
            AGG_RECORDS.inc(processed)
            AGG_RUNS.labels(status="success").inc()
            AGG_LAST_SUCCESS.set(time.time())
            ...
            log.info(
                ...
                metric_date, dau, started, finished, conversion, avg_view,
                len(top), len(retention), elapsed,
            )
            return {
            ...
            AGG_DURATION.observe(time.monotonic() - start_ts)
            AGG_RUNS.labels(status="failure").inc()
            raise
